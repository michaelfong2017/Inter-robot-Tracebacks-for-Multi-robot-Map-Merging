...
void cloud_cb(const PointCloudConstPtr &cloud)
{
    ...
    if (record_continuously || record_single)
    {
        ...
        std::string filename = str(boost::format("%s_%s.pcd") % topic_name % formatted_stamp);
...
int main(int argc, char **argv)
{
    ...
    std::string topic_name;
    private_nh.param<std::string>("topic", topic_name, "");
    ros::Subscriber sub = nh.subscribe(topic_name, queue_size, cloud_cb);
    ...
    while (nh.ok())
    {
        ...
        else
        {
            ...
            m.lock();
            {
                ...
                color_handler.reset(new pcl::visualization::PointCloudColorHandlerCustom<PointCloud2>(cloud_, 211.0, 211.0, 211.0));
